...
def scale(arr):
    ...
    return np.nan_to_num((arr - arr.mean(axis=1, keepdims=True)) / arr.std(axis=1, keepdims=True))
...
def gower(x, y, wgts):
    ...
    def edu_dist(x, y):
        return abs(x - y) / 7  # range of education values

    d = cat_equal(x, y).astype(float)
    d.put(2, edu_dist(x[2], y[2]))
...
def fuse(donors, recipients, linking, fusion, critical=None, imp_wgts=None,
    ...
    if not critical:
        if imp_wgts:  # try to group by variable with highest weight
            critical = linking[imp_wgts.argmax()]
        else:         # take variable with most unique values to ensure smallest groups
            critical = linking[np.argmax([recipients[var].unique().size for var in linking])]

    cc_donors = donors.groupby(critical, sort=False) # turn off sorting is better performance
    ...
    for name, recips_cc in cc_recips:
        ...
        fused_vars = np.apply_along_axis(get_fusion_vars, 1, top_matches, donors_cc, fusion)
    ...
    fused_df = fused_df.append([df for key, df in fused_data.items() if key != 1])

    return fused_df


def main(files, vars_, k=10):
    donors, recipients, maps = load_data(files)

    linking_vars, fusion_vars, critical_vars = map(maps.get, ['linking', 'fusion', 'critical'])

    imp_wgts = importance_wgts(recipients.append(donors)[linking_vars])

    fused_data = fuse(donors, recipients, linking_vars, fusion_vars,
                      critical_vars, imp_wgts, k)

    fused_data.to_csv("fused_data_{}nn-mode.csv".format(k), index=False)


if __name__ == "__main__":

    files = {}
    files['donor'] = 'donors.csv'
    files['recipient'] = 'recipients.csv'
    files['maps'] = 'maps.npy'
    ...
    vars_ = dict(critical_vars = ['age_enc'],
                 ...
                 target = ['y'],
                 fusion_vars = ['contact_enc', 'month_enc', 'day_of_week_enc',
...
#    distances = np.apply_along_axis(gower, 1, cc[linking_vars].values, rr, imp_wgts)
